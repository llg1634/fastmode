...
fn resolve_remote_exports(
    ...
    unsafe {
        ...
        let init_local = GetProcAddress(local_mod, windows::core::s!("Speedhack_InitThread"))
            .ok_or_else(|| "缺少导出 Speedhack_InitThread".to_string())? as usize;
        let set_local = GetProcAddress(local_mod, windows::core::s!("Speedhack_SetSpeedThread"))
            .ok_or_else(|| "缺少导出 Speedhack_SetSpeedThread".to_string())? as usize;
